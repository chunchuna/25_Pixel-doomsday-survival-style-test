...
Unreal__.GameBegin(() => {
    ...
    const initialItems = DeserializeItemsOnly(Unreal__.runtime.globalVars.PlayerInventory);
    // Create update function to update global variables after inventory operations
    const updatePlayerInventory = (items: Item[]) => {
        Unreal__.runtime.globalVars.PlayerInventory = SerializeItemsOnly(items);
    };
...
Unreal__.GameBegin(() => {
    //@ts-ignore
    var initialItems
    Unreal__.runtime.addEventListener("load", (e:any) => {
        initialItems = DeserializeItemsOnly(Unreal__.runtime.globalVars.PlayerInventory);
        ...
        if (inventoryManager) {
            ...
            setTimeout(() => {
                ...
                const control = inventoryManager.BindPlayerMainInventory(
                    DeserializeItemsOnly(Unreal__.runtime.globalVars.PlayerInventory), 
                    30, 
                    6, 
                    "Tab"
                );
